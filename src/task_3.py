...
class Navigation:
    ...
    def init_app(self):
        ...
        # )
        rospy.Subscriber("/camera/rgb/image_raw", ROSImage, self.image_callback)
        # Publishers
        ...
        # CV bridge for using opencv
        self.bridge = CvBridge()
        # For displaying images
        cv2.namedWindow("image", cv2.WINDOW_NORMAL)
        # Image variables
        self.bin_area = 0
        self.target_area = 180000.0
        self.bin_x = 0
        self.bin_detect = False
    ...
    def image_callback(self, data: ROSImage):
        # Read and save image data
        image = self.bridge.imgmsg_to_cv2(data, desired_encoding="bgr8")

        # Color filter for black bin
        ...
        if len(contours) > 0:
            ...
            cx = int(M["m10"] / M["m00"])
            cy = int(M["m01"] / M["m00"])
            image = cv2.circle(image, (cx, cy), 2, (255, 0, 0), 5)
            self.bin_area = cv2.contourArea(contours[0])
            self.bin_x = image.shape[1] / 2 - cx
            print(f"bin x = {self.bin_x}, area = {self.bin_area}")
            if self.bin_area >= self.target_area:
                self.bin_detect = True
        else:
            self.bin_detect = False
        self.show_img(image)
